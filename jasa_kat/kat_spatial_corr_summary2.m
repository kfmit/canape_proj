...
for i=1:4 %(length(freq_array1)-1)
    ...
    gps_site = [72+54.4580/60 , -(157+29.2442/60)]; %gps_site_SHRU5
    dlon=40;
    ...
    for tt=3:loop_end
        ...
        J=squeeze(corr_spa_ave2(:,:,tt));   %from SHRU5
        J(tutu)=NaN;
        %    figure, imagesc(J)
        ...
        J_shru1=squeeze(corr_spa_ave2_shru1(:,:,tt));   %from SHRU1
        J_shru1(tutu)=NaN;
...
%         [ii_shru1 jj_shru1]=ind2sub(size(latitude), indc_ave2_shru1(tt));
        ...
        [R_shru5(tt), Pvalue5(tt)]=corr(SPL_ANL_ok_norm,d_ok_norm,'type','Pearson','rows','all','tail','both');
        ...
        title(['R_{max}=' num2str(R_shru5(tt))])
        xlabel('Days')
        grid on
        ylim([-3 4])
        ...
        % the max corr spot
        shru1_maxcorr_lat(tt)=double(latitude(toto, tata));
        shru1_maxcorr_lon(tt)=double(longitude(toto,tata));
        plotm(double(latitude(toto, tata)),double(longitude(toto,tata)),'xr','markersize',16,'linewidth',3)
        ...
        [R_shru1(tt), Pvalue1(tt)]=corr(SPL_ANL_ok_norm,d_ok_norm,'type','Pearson','rows','all','tail','both');
        ...
        %         datestr(t_beg_num, 'yyyymmdd') '-' datestr(t_end_num, 'yyyymmdd')]  ...
        %         ,'-dpng')
    end     % end of loop going through dates
        savestring = ['combo_50_' num2str(freq_range1) '_' num2str(freq_range2) 'corr']
    save(savestring,'freq_range1','freq_range2','maxcorr_lat','maxcorr_lon','cmax','dist','R_shru5','Pvalue5',...
        'shru1_maxcorr_lat','shru1_maxcorr_lon','cmax_shru1','dist_shru1','R_shru1','Pvalue1')
...
r_shru5=[min(R_shru5(3:loop_end)) mean(R_shru5(3:loop_end)) max(R_shru5(3:loop_end))]
