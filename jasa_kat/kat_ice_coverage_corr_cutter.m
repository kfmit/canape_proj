...
for i = f_ind:f_ind % length(freq_array2)
    % figure(3)
    tfig=tiledlayout(3,3);
...
% 
    ...
    %% second loop to save shit
        ...
        avglat2(tt) = minlat(tt) + ((maxlat(tt)-minlat(tt))/2);
        avglon2(tt) = minlon(tt) + ((maxlon(tt)-minlon(tt))/2);
        ...
        c.Label.String = 'Correlation coefficient';
        caxis([0.4 0.7])
        title(['Correlation over ' num2str(cut_val) ' for ' num2str(freq_range1) '-' num2str(freq_range2) ' Hz'])
        ...
        title([datestr(t_beg_num, 'ddmmmyyyy') ' to ' datestr(t_end_num, 'ddmmmyyyy')])
