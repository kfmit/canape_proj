...
nc_files = dir('/home/julien/Desktop/DataAux/ice_thickness/smos_sea_ice_thickness/*.nc');

lat_thick = ncread(['/home/julien/Desktop/DataAux/ice_thickness/smos_sea_ice_thickness/' nc_files(1).name], 'latitude');
lon_thick = ncread(['/home/julien/Desktop/DataAux/ice_thickness/smos_sea_ice_thickness/' nc_files(1).name], 'longitude');
...
for t_num_loop=t0_num:15:t1_num
    ...
    %%%% find ice thickness file
    for nn=1:size(nc_files,1)
        nameFile=nc_files(nn).name;
        if strcmp(nameFile(end-10:end-3), ddd_mid)
            n_ok=nn;
        end
    end
    
    nameFile=nc_files(n_ok).name;
    sea_ice_thickness = ncread(['/home/julien/Desktop/DataAux/ice_thickness/smos_sea_ice_thickness/' nameFile], 'sea_ice_thickness');
    uncertainty = ncread(['/home/julien/Desktop/DataAux/ice_thickness/smos_sea_ice_thickness/' nameFile], 'ice_thickness_uncertainty');

    
    sea_ice_thickness(sea_ice_thickness<0)=NaN;
    sea_ice_thickness(uncertainty>1)=NaN;
    [Gmag,Gdir] = imgradient(sea_ice_thickness);
    ...
    %%% plot data
    surfm(double(lat_thick), double(lon_thick), sea_ice_thickness)
...
%     caxis(ccc)
    ...
    %%% plot data
    surfm(double(lat_thick), double(lon_thick), Gmag)
    ...
    colorbar
%     caxis(ccc)
    title(['Ice thickness gradient'])      
     
    
    print(gcf,['./spatial_corr_result/' sitename '_interp_comp_thick/spatial_corr_' ...
