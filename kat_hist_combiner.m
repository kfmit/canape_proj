...
%% separate by mode
ANL_duct_mode = zeros(1,39);
ANL_no_duct_mode = zeros(1,39); 
ANL_no_ice_mode = zeros(1,39);
for i = 1:39
ANL_no_duct_mode(i) = mode(ANL_no_duct(:,i));
ANL_no_ice_mode(i) = mode(ANL_no_ice(:,i));
ANL_duct_mode(i) = mode(ANL_duct(:,i));
end


figure
p1 = plot(avg_freq,ANL_duct_mode,'-o','MarkerEdgeColor','#4DBEEE','MarkerFaceColor','#4DBEEE')
hold on
p2 = plot(avg_freq,ANL_no_duct_mode,'-o','MarkerEdgeColor','#D95319','MarkerFaceColor','#D95319')
p3 = plot(avg_freq,ANL_no_ice_mode,'-o','MarkerEdgeColor','#EDB120','MarkerFaceColor','#EDB120')
...
%% separate by biggest bin (this is mode right)
N_duct=zeros(1,39); N_no_duct=zeros(1,39); N_no_ice=zeros(1,39);
edges_duct=zeros(1,39);

for i=1:39
[N_duct,edges_duct] = histcounts(ANL_duct(:,i));
[N_no_duct,edges_no_duct] = histcounts(ANL_no_duct(:,i));
[N_no_ice,edges_no_ice] = histcounts(ANL_no_ice(:,i));

figure
histogram2(ANL_no_ice(:,1))
