...
t_beg=[2016,12,31,00,00,00];  %%% date vector [YY, MM, DD, h, min, sec]
t_end=[2017,03,01,00,00,00];   %%% sa
...
t=t(ind_t_ok);
Nt=length(t);
auxData_t_psd=auxData_t_psd(ind_t_ok,:);
SPL_ANL=SPL_ANL(ind_t_ok,:);
SPL_raw=SPL_raw(ind_t_ok,:);
...
for ff=1:Nf
    for ii=1:Nvar
        ind_no_nan=~isnan(auxData_t_psd(:,ii));
        %%%% remove corr data which are nearly constant and/or nearly all
        %%%% NaN
        if length(unique(auxData_t_psd(ind_no_nan,ii)))>10
            X = [vecSPL(ind_no_nan,ff), auxData_t_psd(ind_no_nan,ii)];
            X_norma=zscore(X,[],1);
