...
%     title(['2-day averaged SPL - Max corr = ' num2str(R(tt))])  
    title(['SHRU5 - ' datestr(t_beg_num, 'dd mmm yyyy') ' to ' datestr(t_end_num, 'dd mmm yyyy')])
...
%    d_ok_interp=interp1(t_osisaf(ind_t_ok_osisaf), d_ok, t(ind_t_ok), 'nearest');
   t_ok=t_osisaf(ind_t_ok_osisaf); %%% common time axis
    ...
    title(['SHRU5 - R_{max}=' num2str(R(tt))])
    xlabel('Days')
    grid on
    ...
    caxis(ccc)
%     title(['2-day averaged SPL - Max corr = ' num2str(R(tt))])  
    title(['SHRU1 - ' datestr(t_beg_num, 'dd mmm yyyy') ' to ' datestr(t_end_num, 'dd mmm yyyy')])
...
%    d_ok_interp=interp1(t_osisaf(ind_t_ok_osisaf), d_ok, t(ind_t_ok), 'nearest');
   t_ok=t_osisaf(ind_t_ok_osisaf); %%% common time axis
    ...
    subplot(224)
    plot(t_ok-t_ok(1),(SPL_ANL_ok-mu_spl)/sigma_spl, 'Color', [0.4940    0.1840    0.5560], 'linewidth',2)
       
    hold on
    for bb=1:nb
        plot(t_ok(ind_b{bb})-t_ok(1), block{bb}, '-', 'Color', [ 0.6350    0.0780    0.1840], 'linewidth',2)
        hold on
    end
    
    plot(t_ok(ind_no_nan)-t_ok(1), d_ok_norm, 'o', 'Color', [ 0.6350    0.0780    0.1840], 'linewidth',2)
    title(['SHRU1 - R_{max}=' num2str(R_shru1(tt))])
    xlabel('Days')
    grid on
        ...
        print(gcf,['./spatial_corr_result/plot_with_SHRU5_SHRU1/spatial_corr_' ...
