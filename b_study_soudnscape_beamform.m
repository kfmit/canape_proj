...
%%%%%%%
%%% There is a bug in the timestamp_wavDataFiles computation. It is corrected
%%% l.110. Make sure to remove this once bug is corrected

%% Code options
%%%% name of the LTSA to study
namefile='CANAPE_SHRU5_914_beamform_sw_0.0625_lw_420_osw_0.03125_olw_0_theta_0_45_90_135_180';
...
t_beg=[2017,01,01,00,00,00];  %%% date vector [YY, MM, DD, h, min, sec]
t_end=[2017,03,01,00,00,00];   %%% same
...
path_Results = ['.' filesep 'Results_Beamform' filesep];
...
%%%%% study a single percentile
pp0=3;
vPSD_kinda=squeeze(vPSD_kinda(:,:,pp0,:));

for pp=1:Ntheta


    vPSD_pp=squeeze(vPSD_kinda(:,:,pp));
    name_pp=['_theta_' num2str(theta(pp))];

    
%% fusion of PSD with multiple ocean observation sources        
    disp(['Merging Acoustics and Environnemental data, angle ' ... 
        num2str(pp) '/' num2str(Ntheta)])
    ...
    disp(['Acoustic and Environmental data merged in ', num2str(elapsetipeMergingAux), ' sec ; angle ' ... 
        num2str(pp) '/' num2str(Ntheta)])
    ...
    disp(['Plotting LTSA, angle ', num2str(pp), '/', num2str(Ntheta)])
    ...
    FormatFigures([path_ResultsFigures filesep 'LTSA' name_pp], save_matlab_fig)

    disp(['LTSA saved, angle ', num2str(pp), '/', num2str(Ntheta)])

    end
    %% empirical proba density   
    if computeEPD

    disp(['Computing spectrum probability density, angle ', num2str(pp), '/', num2str(Ntheta)])
    figure('visible','off');
    A=10*log10(vPSD);        
    prctilePlot_jb;
    FormatFigures([path_ResultsFigures filesep 'EPD' name_pp], save_matlab_fig)
    disp(['Spectrum probability density saved,  angle ', num2str(pp), '/', num2str(Ntheta)])

    end
    %% scattering plots  
    if plotScatt

    disp(['Making scatter plots, channel, angle ', num2str(pp), '/', num2str(Ntheta)])
    ...
    FormatFigures([path_ResultsFigures filesep 'scatAcoust' name_pp], save_matlab_fig)
    ...
    disp(['Scatter plots saved, angle ',num2str(pp), '/', num2str(Ntheta)])

    end
    %% scattering and pearson correlation coefs
    if computeCorr

    disp(['Computing correlations, angle ', num2str(pp), '/', num2str(Ntheta)])
    ...
    figure('visible','off');   
    SubplotCorr;       
    FormatFigures([path_ResultsFigures filesep 'PairWiseCorr' name_pp], save_matlab_fig)

    disp(['Correlations saved, angle ', num2str(pp), '/', num2str(Ntheta)])
...
%                 T=readtable([path_auxData 'info_variables_icemotion.csv']);
        ...
        for ff=1:Nband
            ...
            disp(['Computing spatial correlations, freq ' num2str(f1), '-', num2str(f2), ' Hz', ', angle ', num2str(pp), '/', num2str(Ntheta)])
...
%                 plot_spatial_corr;
            ...
            FormatFigures([path_ResultsFigures filesep 'SpatialPearson_freq_' num2str(f1) '-' num2str(f2) name_pp], save_matlab_fig)

            disp(['Spatial correlations saved, freq ' num2str(f1), '-', num2str(f2), ' Hz', ', angle ', num2str(pp), '/', num2str(Ntheta)])
    ...
    if computePCA
        disp(['Computing PCA, angle ', num2str(pp), '/', num2str(Ntheta)])
        ...
        grid on 
        FormatFigures([path_ResultsFigures filesep 'PCA' name_pp], save_matlab_fig)

        disp(['PCA saved, angle ', num2str(pp), '/', num2str(Ntheta)])
    end   
end
