...
%%%%% and with nohup
%%% nohup /usr/local/bin/matlab -nodisplay -nosplash -r a_compute_PSD_a_la_Roth > matlab_nohup_Roth.out &
...
nFFT_sec= 1; % short window, in seconds
nIntegWin_sec= 200; % long window, in seconds

nOverlapFFT_sec = 0; % in second
nOverlapIntegWin_sec = 0; % in seconds

step_IntegWin = 73; % One IntegWin every step_IntegWin file will be processed to compute the LTSA. 
...
namefile=strcat(SHRU_SN,'_sw_',num2str(nFFT_sec),'_lw_',num2str(nIntegWin_sec),'_osw_',num2str(nOverlapFFT_sec),'_olw_',num2str(nOverlapIntegWin_sec));
