...
for nn=1:length(n_ok)
    ...
    toto=ncinfo(['./smos_sea_ice_thickness/' nameFile]);
    
    
    sea_ice_thickness{nn} = ncread(['./smos_sea_ice_thickness/' nameFile], 'sea_ice_thickness');
    uncertainty{nn} = ncread(['./smos_sea_ice_thickness/' nameFile], 'ice_thickness_uncertainty');
...
for nn=3:length(n_ok)
    subplot(2,3,nn-2)
    thickness=sea_ice_thickness{nn};
    thick_un=uncertainty{nn};
    
    thickness(thickness<0)=NaN;
    thickness(thick_un>1)=NaN;
