...
%     title(datestr(date_num_ok(nn)))
    ...
    geoshow(maph, land, 'FaceColor',[0.80 0.80 0.80],'EdgeColor',0.30*[1 1 1]);    
    %%% add ice data
    d_div_ok=squeeze(d_div(nn,:,:));
    d_div_ok(tutu)=NaN;
    surfacem(double(latitude),double(longitude),abs(d_div_ok))
    ...
    geoshow(maph, land, 'FaceColor',[0.80 0.80 0.80],'EdgeColor',0.30*[1 1 1]);    
    %%% add ice data
    d_curl_ok=squeeze(d_curl(nn,:,:));
    d_curl_ok(tutu)=NaN;
    surfacem(double(latitude),double(longitude),abs(d_curl_ok))
    ...
    NameFig=['./fig_drift_new/' datestr(date_num_ok(nn),'yymmdd')];
