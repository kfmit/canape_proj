...
for nn=deb:deb+5
    ...
    d_temp=zeros(size(latitude));
    n_ave=0;
    for mm=ave_vec
        nameFile=nc_files(n_ok(nn)+mm).name;
        pathFile=nc_files(n_ok(nn)+mm).folder;
...
%         datevect{ind,1} = date_vec_ok;
        ...
        %%% check for too much NaN
        toto=find(isnan(ddd));       
        if length(toto) < 0.95*length(ddd(:))       
            d_temp=d_temp+ddd;
            n_ave=n_ave+1;
        end
    end
...
for nn=1:6
    ...
    title([datestr(date_num_ok(nn)) ' \pm 15 days'])
