...
%% March 11 and March 12 are bad and must be removed
%%%%% these are indexes 163 and 164
ind=163:164;
deltalat(ind,:,:)=NaN;
deltalon(ind,:,:)=NaN;
d(ind,:,:)=NaN;
...
nn0=33; %%% nn=33 is Nov 1st
delta_nn=15; %%% one average every 15 days
dn=60; %%% average over 60 days

ind=1;
for nn=nn0:delta_nn:size(nc_files,1)-60
    ...
    deltalat_ok=squeeze(nanmean(deltalat(nn:nn+60,:,:),1));
    deltalon_ok=squeeze(nanmean(deltalon(nn:nn+60,:,:),1));
...
%     title(datestr(date_num_ok(nn)))
    ...
    geoshow(maph, land, 'FaceColor',[0.80 0.80 0.80],'EdgeColor',0.30*[1 1 1]);    
    %%% add ice data
    d_ok=squeeze(nanmean(d(nn:nn+60,:,:),1));
    d_ok(tutu)=NaN;
    surfacem(double(latitude),double(longitude),d_ok)
    ...
    geoshow(maph, land, 'FaceColor',[0.80 0.80 0.80],'EdgeColor',0.30*[1 1 1]);    
    %%% add ice data
    d_div_ok=squeeze(nanmean(abs(d_div(nn,:,:)),1));
    d_div_ok(tutu)=NaN;
    surfacem(double(latitude),double(longitude),d_div_ok)
    
    %%% add mooring
    plotm(gps_site(1), gps_site(2), 'xk','markersize',16,'linewidth',3)
    title('Drift divergence')    
    ...
    geoshow(maph, land, 'FaceColor',[0.80 0.80 0.80],'EdgeColor',0.30*[1 1 1]);    
    %%% add ice data
    d_curl_ok=squeeze(nanmean(abs(d_curl(nn,:,:)),1));
    d_curl_ok(tutu)=NaN;
    surfacem(double(latitude),double(longitude),d_curl_ok)
    ...
    geoshow(maph, land, 'FaceColor',[0.80 0.80 0.80],'EdgeColor',0.30*[1 1 1]);    
    %%% add ice data
    d_shear_ok=squeeze(d_shear(nn,:,:));
    d_shear_ok(tutu)=NaN;
    surfacem(double(latitude),double(longitude),abs(d_shear_ok))
    ...
    NameFig=['./fig_drift_moy/' datestr(date_num_ok(nn),'yyyymmdd') '-' datestr(date_num_ok(nn+60),'yyyymmdd')];
